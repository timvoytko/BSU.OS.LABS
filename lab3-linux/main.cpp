...
std::vector<std::vector<pthread_mutex_t>> mutexes(0);
void* thread_multiply_by_row_and_col(void* lp_param) {
    //row and column
    int* arr = (int*)lp_param;
    int r = arr[0];
    int c = arr[1];
...
void* thread_multiply_by_col_and_row(void* lp_param) {
    // col and row
    int* arr = (int*)lp_param;
    int c = arr[0];
    int r = arr[1];
...
void* thread_multiply_by_block(void* lp_param) {
...
public:
    ThreadMatrixMultiplier(int number_of_threads) {
        ...
        mutexes = std::vector<std::vector<pthread_mutex_t>>(a.get_n());
        for (int i = 0; i < a.get_n(); i++) {
            mutexes[i].resize(b.get_m());
            for (int j = 0; j < b.get_m(); j++) {
                mutexes[i][j] = pthread_mutex_t();
            }
        }
    }
    BasicMatrix  multiply_by_row_and_column() {
        ...
        for (int i = 0; i < a.get_n() * b.get_m(); i++) {
            ...
            if (count_of_threads < number_of_threads_) {
                ...
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[i], NULL, thread_multiply_by_row_and_col, p);
                ...
                pthread_join(threads_arr[l], (void**)&status_addr);
                l++;
                int* arr = new int[2];
                arr[0] = tmp.first;
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[l+this->number_of_threads_-1], NULL, thread_multiply_by_row_and_col, p);
                if (status != 0) {
                    std::cout << "main error: can't create thread\n";
                    std::cout<<status;
                    exit(-11);
        ...
        for (int i = a.get_n() * b.get_m() - number_of_threads_; i < a.get_n() * b.get_m()-1; i++) {
            pthread_join(threads_arr[i], nullptr);
        }
    ...
    BasicMatrix multiply_by_column_and_row() {
        ...
        for (int i = 0 ; i < a.get_m() * b.get_n(); i++) {
            ...
            if (count_of_threads < number_of_threads_) {
                ...
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[i], NULL, thread_multiply_by_col_and_row, p);
                ...
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[l+this->number_of_threads_-1], NULL, thread_multiply_by_col_and_row, p);
                if (status != 0) {
                    std::cout << "main error: can't create thread\n";
                    std::cout<<status;
                    exit(-11);
    ...
    BasicMatrix multiply_by_block() {
        ...
        for (int i = 0; i < is_and_js.size(); i++) {
            ...
            if (count_of_threads < number_of_threads_) {
                ...
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[i], NULL, thread_multiply_by_block, p);
                ...
                arr[1] = tmp.second;
                void* p = arr;
                status = pthread_create(&threads_arr[l+this->number_of_threads_-1], NULL, thread_multiply_by_block, p);
                if (status != 0) {
                    std::cout << "main error: can't create thread\n";
                    std::cout<<status;
                    exit(-11);
